    ...
    if (!responseXML) {
      ...
      for (pm = 0; pm < placemarkNodes.length; pm++) {
        ...
        if (!!extDataNodes && extDataNodes.length > 0) {
          ...
          var simpleDataNodes = getElementsByTagName(extDataNodes[0], 'SimpleData');
          for (var d = 0; d < simpleDataNodes.length; d++) {
            var dn    = simpleDataNodes[d];
            var name  = dn.getAttribute('name');
            if (!name) continue;
            if (!dn.childNodes[0]) continue;
            var val   = dn.childNodes[0].nodeValue;

            placemark.vars.val[name]     = val;
            placemark.vars.display[name] = name;
          }
        }
